...
function train!(
        ...
        cbeval::Function=function cbeval(model, dataloader)
            @info "eval" evaluate(model, dataloader)... log_step_increment=0
        end,
        cbinit::Function=function cbinit(model, dataloader)
            @info "train" loss(model, first(dataloader.train))... log_step_increment=0
            cbeval(model, dataloader)
        end,
...
function savemodel(model::Trainable, modelpath="$(getlogger(model).logdir)/model.bson"; verbose=true)
    ...
    verbose && @info "Saved model at iteration $(getiter(model)) to $modelpath"
    return modelpath
end
...
function loadmodel!(model::Trainable, modelpath::String; verbose=true)
    ...
    iter = file[:iter]
    setiter!(model, iter)
    verbose && @info "Loaded model at iteration $iter from $modelpath"
end
